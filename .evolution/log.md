...
| 2026-05-12 | AgentBoss | 🤖 Self-evolution: process_manager.py — add %s exception repr to 8 logger.error calls (write×2, read×2, resize×2, create×2); all error logs now show the actual exception message + full traceback; previously 8 sites had exc_info=True but no %s so the exception string was silently discarded | 中：PTY 进程 write/read/resize/create 失败现在有完整错误信息可追溯，不再只输出 traceback 而没有异常内容 |
