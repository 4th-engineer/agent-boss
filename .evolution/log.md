...
|| 2026-05-11 | AgentBoss | 🤖 Self-evolution: map_view.py — fix ZeroDivisionError in refresh_map when viewport not yet laid out (vp_width=0 → cols=0 → i%0 crash); extract vp_width variable for clarity + ternary guard for cols | 中：Worlds map 在 Qt 初始化阶段调用 refresh_map 不再崩溃 |
