...
|------------|------------|----------------------------------------------------|-----------------|
| 2026-05-12 | AgentBoss | 🤖 Self-evolution: map_view.py — replace bare `except Exception` + `logger.error` (silent swallow) with typed `(RuntimeError | OSError | ValueError)` + `logger.critical` + `raise`; refresh_map now propagates room data failures instead of silently showing blank map | 中：修复静默错误吞掉，提升可观测性 |
